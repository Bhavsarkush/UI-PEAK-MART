...
class _AccountUserState extends State<AccountUser> {
          ...
          child: Center(
            child: Column(
              children: [
                ...
                _readOnlyTextField("Username", _userData["Name"] ?? ""),
                const SizedBox(height: 20),
                _readOnlyTextField("Email", _userData["Email"] ?? ""),
                const SizedBox(height: 20),
                _readOnlyTextField("Mobile Number", _userData["Mobile"] ?? ""),
                const SizedBox(height: 20),
                _readOnlyTextField("Password", _userData["Password"] ?? "", obscureText: true),
                const SizedBox(height: 20),
              ],
            ),
          ),
  ...
  Widget _readOnlyTextField(String label, String value, {bool obscureText = false}) {
    return TextFormField(
      readOnly: true,
      initialValue: value,
      decoration: InputDecoration(
        labelText: label,
        border: OutlineInputBorder(borderRadius: BorderRadius.circular(20)), // Adding an outline border
        labelStyle: const TextStyle(
          fontSize: 18, // Slightly reduced for better spacing
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Colors.cyan,
        centerTitle: true,
        title: const Text('Edit User Information',style: TextStyle(fontWeight: FontWeight.bold),),
      ),
